...
while i <= len(mon):
    ...
    if i ==0:
        ...
        print(tempInf)
```
![image](https://user-images.githubusercontent.com/114161306/194026860-8c4f3db8-4180-4ea7-b77c-a0076468c226.png)


## Задание 3

Я записала фразы "Переменная loss больше 1000, милорд", "переменная loss меньше 1000, милорд" и "переменная loss стремится к нулю, милорд" на диктофон и заменила ими оригинальные аудиофайлы. А ещё поменяла таблицу из которой берут данные. Всё остальное осталось таким же.
```py
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    IEnumerator GoogleSheets()
    {
        UnityWebRequest currentResp = UnityWebRequest.Get("https://sheets.googleapis.com/v4/spreadsheets/1oGfAHKsDBnTtgq7IYbe_dx7XzxREH-QNCT3Kk66ZAMo/values/Лист1?key=AIzaSyA8zCEkBToRTP-ZCfH1FT_4mXjN7XIBdiw");
        yield return currentResp.SendWebRequest();
        string rawResp = currentResp.downloadHandler.text;
        var rawJson = JSON.Parse(rawResp);
        foreach (var itemRawJson in rawJson["values"])
...
## Выводы
В этой лабораторной я связала Pycharm с Google таблицами и Unity, пользуясь Google Cloud. Первое задание требовало кропотливого переписывания каждого символа из видео с инструкцией, второе и третье - определённой самостоятельности. И если в PyCharm всё шло довольно легко, то в Unity после добавления других аудиофайлов и нового скрипта сломался первый. Возможно, стоило создать под это другой проект.
